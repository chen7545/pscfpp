...
namespace Fd1d
{
                                    ...
                                    Array<double>& residual)
   {
      ...
      for (i = 0; i < nm; ++i) {
         for (j = 0; j < nm; ++j) {
            DArray<double> cField = cFields[j];
            DArray<double> wField = wFields[j];
            chi = interaction_.chi(i,j);
            p   = interaction_.p(i,j);
            for (k = 0; k < nx; ++k) {
               int idx = i*nx + k;
               residual[idx] += chi*cField[k] - p*wField[k];
            }
         }
      }
   ...
   */
   ...
   void NrIterator::getResidual(Array<WField> const & wFields,
                                Array<CField> const & cFields,
                                Array<double>& residual)
   {
      const int nm = system().mixture().nMonomer();
   ...
   {   // Start overall timer
      ...
      //getResidual(system().wFields(), system().cFields(), resid_);
      ...
      for (i = 0; i < maxItr_; ++i) {
         ...
         system().mixture().compute(wFieldsNew_, cFieldsNew_);
         computeResidual(wFieldsNew_, cFieldsNew_, residualNew_);
         //getResidual(wFieldsNew_, cFieldsNew_, residNew_);
         ...
         while (normNew > norm && j < 3) {
            ...
            system().mixture().compute(wFieldsNew_, cFieldsNew_);
            computeResidual(wFieldsNew_, cFieldsNew_, residualNew_);
            //getResidual(wFieldsNew_, cFieldsNew_, residualNew_);
            normNew = residualNorm(residualNew_);
            ++j;
         }
