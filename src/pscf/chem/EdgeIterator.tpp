...
namespace Pscf {
   ...
   */
   ...
   EdgeIterator<WT>::EdgeIterator(PolymerSpecies<WT> const & polymer)
    : currentVertexId_(-1),
      targetVertexId_(-1),
      polymerPtr_(&polymer)
   {}
   ...
   */
   template <typename WT>
   void EdgeIterator<WT>::begin(int sourceId, int targetId)
   {
   ...
   */
   template <typename WT>
   EdgeIterator<WT>& EdgeIterator<WT>::operator ++ ()
   {
      UTIL_CHECK(notEnd());
   ...
   */ 
   template <typename WT>
   int EdgeIterator<WT>::currentEdgeId() const
   ...
   */ 
   template <typename WT>
   int EdgeIterator<WT>::currentDirectionId() const
   ...
   */ 
   template <typename WT>
   int EdgeIterator<WT>::currentVertexId() const
   ...
   */
   template <typename WT>
   bool EdgeIterator<WT>::isEnd() const
   {
   ...
   */
   template <typename WT>
   bool EdgeIterator<WT>::notEnd() const
   {
      return (bool)( currentVertexId_ != targetVertexId_ );
   }
