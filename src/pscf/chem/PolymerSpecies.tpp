...
namespace Pscf
{
   ...
   */
   template <typename WT>
   void PolymerSpecies<WT>::readParameters(std::istream& in)
   {
      // Read polymer type (linear by default)
      type_ = PolymerType::Linear;
      ParamComposite::readOptional<PolymerType::Enum>(in, "type", type_);

      ParamComposite::read<int>(in, "nBlock", nBlock_);
   ...
   */
   template <typename WT>
   void PolymerSpecies<WT>::makePlan()
   {
      if (nPropagator_ != 0) {
   ...
   */
   template <typename WT>
   void PolymerSpecies<WT>::makePaths()
   {
   ...
   */
   template <typename WT>
   void PolymerSpecies<WT>::isValid()
   {
      Pair<int> pair;
   ...
   */
   template <typename WT>
   double PolymerSpecies<WT>::length() const
   {
   ...
   */
   template <typename WT>
   int PolymerSpecies<WT>::nBead() const
   {
