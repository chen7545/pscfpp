#ifndef PSPC_LR_AM_CORRECTION_COMPRESSOR_TPP
#define PSPC_LR_AM_CORRECTION_COMPRESSOR_TPP
...
namespace Pspc{
   ...
   // Constructor
   template <int D>
   LrAmCorrectionCompressor<D>::LrAmCorrectionCompressor(System<D>& system)
    : Compressor<D>(system),
      isAllocated_(false)
   {  setClassName("LrAmCorrectionCompressor"); }

   // Destructor
   template <int D>
   LrAmCorrectionCompressor<D>::~LrAmCorrectionCompressor()
   {  setClassName("LrAmCorrectionCompressor"); }

   // Read parameters from file
   template <int D>
   void LrAmCorrectionCompressor<D>::readParameters(std::istream& in)
   {
   ...
   template <int D>
   void LrAmCorrectionCompressor<D>::setup(bool isContinuation)
   {  
      ...
      if (!isAllocated_){
         ...
         residM_.allocate(dimensions);
         residKM_.allocate(dimensions);
         intraCorrelation_.allocate(kMeshDimensions_);
         fCorrections_.allocate(kMeshDimensions_);
   ...
   template <int D>
   void LrAmCorrectionCompressor<D>::setEqual(DArray<double>& a, DArray<double> const & b)
   {  a = b; }

   // Compute and return inner product of two vectors.
   template <int D>
   double LrAmCorrectionCompressor<D>::dotProduct(DArray<double> const & a, 
                                    DArray<double> const & b)
   {
      ...
      for (int i = 0; i < n; i++) {
         ...
         if (std::isnan(a[i]) || std::isnan(b[i])) { 
            throw NanException("LrAmCorrectionCompressor::dotProduct",
                               __FILE__,__LINE__,0);
         }
         product += a[i] * b[i];
      }
   ...
   template <int D>
   double LrAmCorrectionCompressor<D>::maxAbs(DArray<double> const & a)
   {
      const int n = a.capacity();
      double max = 0.0;
      double value;
      for (int i = 0; i < n; i++) {
         ...
         if (std::isnan(value)) { // if value is NaN, throw NanException
            throw NanException("LrAmCorrectionCompressor::dotProduct",__FILE__,__LINE__,0);
         }
         if (fabs(value) > max) {
            max = fabs(value);
         }
   ...
   template <int D>
   void 
   LrAmCorrectionCompressor<D>::updateBasis(RingBuffer< DArray<double> > & basis,
   ...
   template <int D>
   void
   LrAmCorrectionCompressor<D>::addHistories(DArray<double>& trial,
   ...
   template <int D>
   void 
   LrAmCorrectionCompressor<D>::addPredictedError(DArray<double>& fieldTrial,
                                        ...
                                        double lambda)
   {
      ...
      for (int i = 0; i < n; i++) {
         residM_[i] = resTrial[i];
      }
      // Convert residual to Fourier Space
      system().fft().forwardTransform(residM_, residKM_);
      
      // Add Gaussian function correction
      MeshIterator<D> iter;
      iter.setDimensions(residK_.dftDimensions());
      for (iter.begin(); !iter.atEnd(); ++iter) {
         residKM_[iter.rank()][0] /= (1.0 + fCorrections_[iter.rank()]);
         residKM_[iter.rank()][1] /= (1.0 + fCorrections_[iter.rank()]);
      }
      
      // Convert back to real Space
      system().fft().inverseTransform(residKM_, residM_);
      for (int i = 0; i < n; i++) {
         fieldTrial[i] += lambda * residM_[i];
      }
   }

   // Does the system have an initial field guess?
   template <int D>
   bool LrAmCorrectionCompressor<D>::hasInitialGuess()
   {  return system().w().hasData(); }

   // Compute and return the number of elements in a field vector
   template <int D>
   int LrAmCorrectionCompressor<D>::nElements()
   {  return system().domain().mesh().size(); }

   // Get the current field from the system
   template <int D>
   void LrAmCorrectionCompressor<D>::getCurrent(DArray<double>& curr)
   {
   ...
   template <int D>
   void LrAmCorrectionCompressor<D>::getResidual(DArray<double>& resid)
   {
   ...
   template <int D>
   void LrAmCorrectionCompressor<D>::update(DArray<double>& newGuess)
   {
   ...
   template<int D>
   void LrAmCorrectionCompressor<D>::outputToLog()
   {}
   
   template<int D>
   void LrAmCorrectionCompressor<D>::outputTimers(std::ostream& out)
   {
   ...
   template<int D>
   void LrAmCorrectionCompressor<D>::clearTimers()
   {
   ...
   template<int D>
   double LrAmCorrectionCompressor<D>::computeDebye(double x)
   {
      if (x == 0){
         return 1.0;
      } else {
   ...
   template<int D>
   double LrAmCorrectionCompressor<D>::computeIntraCorrelation(double qSquare)
   {
   ...
   template<int D>
   void LrAmCorrectionCompressor<D>::computeIntraCorrelation()
   {
      MeshIterator<D> iter;
   ...
   // Compute f(q) = A * e^{-(q-q*)^2 / (2sigma^2)}
   template<int D>
   double LrAmCorrectionCompressor<D>::fCorrection(double q)
   {
      double sigma = 0.5 * qs_;
      double exponent = -((q-qs_)* (q-qs_)/ (2 * sigma * sigma) );
      //Log::file() << A_ << std::endl;
      return A_* exp(exponent);
   }
   
   template<int D>
   void LrAmCorrectionCompressor<D>::fCorrections()
   {
      MeshIterator<D> iter;
      iter.setDimensions(kMeshDimensions_);
      IntVec<D> G, Gmin;
      ...
      for (iter.begin(); !iter.atEnd(); ++iter) {
         ...
         Gmin = shiftToMinimum(G, system().mesh().dimensions(), 
                                  system().unitCell());
         Gsq = system().unitCell().ksq(Gmin);
         //Log::file()<< sqrt(Gsq) << std::endl;
         fCorrections_[iter.rank()] = fCorrection(sqrt(Gsq));
         //Log::file() << fCorrections_[iter.rank()] << std::endl;
      }
   }
   ...
   template<int D>
   double LrAmCorrectionCompressor<D>::computeError(int verbose)
   {
