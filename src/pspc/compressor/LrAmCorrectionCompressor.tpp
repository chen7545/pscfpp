...
namespace Pspc{
   ...
   void LrAmCorrectionCompressor<D>::getResidual(DArray<double>& resid)
   {
      ...
      for (iter.begin(); !iter.atEnd(); ++iter) {
         residK_[iter.rank()][0] *= 1.0 / (vMonomer * (1 + fCorrections_[iter.rank()]) * intraCorrelation_[iter.rank()]);
         residK_[iter.rank()][1] *= 1.0 / (vMonomer * (1 + fCorrections_[iter.rank()]) * intraCorrelation_[iter.rank()]);
      }
